(function () {
  ...
  function ionGallery($ionicPlatform, ionGalleryHelper, ionGalleryConfig) {
    ...
    function controller($scope) {
      ...
      _drawGallery();
      console.log("watch ", $scope.gallaryId)

      $scope.$watchCollection('ionGalleryItems', function(newNames, oldNames) {
        console.log("fire", newNames)
        _drawGallery();
      });

      // $scope.$watch(function () {
      //   //return $scope.gallaryId;
...
if(!items) return [];
      for(var i=0;i<items.length;i++){
        ...
        //if(!items[i]) break;
        if(items[i].id && items[i].secret){
          // photo.src = "http://farm" + photo.farm + ".static.flickr.com/" + photo.server + "/" + photo.id + "_" + photo.secret + "_s.jpg";

          var url = "http://farm" + items[i].farm + ".static.flickr.com/" + items[i].server + "/" + items[i].id + "_" + items[i].secret;
          items[i].thumb = url + "_m.jpg";
          items[i].src = url + "_b.jpg";
